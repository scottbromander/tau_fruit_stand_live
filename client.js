...
commodities, specifically the name and the ‘market price’ of each. This
information will need to be displayed in a meaningful way on the DOM.
- Object
  - name
  - Market price
  - Method: Change Price
...
interval. Any given fruit is not allowed to go below a cost of 50 cents, or
above the cost of 9 dollars and 99 cents.
- Max price
- Min price
- Price Swing (1 - 50)
...
Meaning that by clicking on the display for each of the fruits, allows the user
to ‘buy’ one of the fruits, at market price, which will be deducted from the
...
one of their fruits of the same type at the current market price. This will also
remove one from their inventory. The user should be not able to sell fruits they
do not already own.
Pro Mode
...
var STARTING_FRUIT_PRICE = 6;
var STARTING_CASH = 100;

var fruitArray = ["Apples", "Oranges", "Bananas", "Grapes", "Kiwi"];
...
var player = new Player(STARTING_CASH);
console.log("HERE: ", player);
// constructor function for the fruits
...
function init(){
  console.log("CATS!");
  createFruits(fruitArray); //converts the array from String to Object
  appendFruitToDOM(fruitArray); //This takes in array of Objects and puts on DOM
  enable();
}

function enable(){
  $("#fruitContainer").on("click", ".fruit-button", clickBuyFruit);
}
...
function createFruits(justAnArray){
  //use an array, to create Fruit Objects
  for(var i = 0; i < justAnArray.length; i++){
    player["inv" + justAnArray[i]] = 0;
    var newFruit = new Fruit(justAnArray[i], STARTING_FRUIT_PRICE);
    justAnArray[i] = newFruit;

  }
  console.log(player);
}

function appendFruitToDOM(justAnArray){
  for(var i = 0; i < justAnArray.length; i++){
    var fruit = justAnArray[i];
    ...
    $el.data("marketprice", fruit.marketPrice);
  }
}

function clickBuyFruit(){
  var fruit = $(this).data();
  if(player.totalCash >= fruit.marketprice){
    player["inv" + fruit.name]++;
    player.totalCash -= fruit.marketprice;
  }
  updatePlayerDisplay();
  console.log(player);
}
...
function updatePlayerDisplay(){
  console.log("Meow ", player.totalCash);
  $("#playerCash").text(player.totalCash);
}
